...
export class CalendarIntegrationService {
  ...
  async getGoogleCalendarTokenByUser(userId: string): Promise<{
    ...
    try {
      ...
      throw new Error(`获取用户 ${userId} 的Google Calendar令牌失败: ${errorMessage}`);
    }
  }
   ...
   * 检查用户是否有有效的日历令牌
   */
  async hasValidCalendarToken(userId: string, provider: string = 'google'): Promise<boolean> {
    try {
      const token = await this.calendarTokenService.getUserToken(userId, provider);
      if (!token) return false;

      const isExpiringSoon = await this.calendarTokenService.isTokenExpiringSoon(userId, provider);
  ...
  async prepareCalendarApiParams(userId: string): Promise<{
    ...
    if (isExpiringSoon) {
      const refreshed = await this.calendarTokenService.refreshToken(userId, 'google');
