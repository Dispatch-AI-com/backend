...
export class AwsSnsSmsVerificationService {
  ...
  async sendVerificationSms(phoneNumber: string, verificationCode: string): Promise<{ success: boolean; message?: string }> {
    try {
      const accessKeyId = this.configService.get<string>('AWS_ACCESS_KEY_ID');
      const secretAccessKey = this.configService.get<string>('AWS_SECRET_ACCESS_KEY');

      if (!accessKeyId || !secretAccessKey) {
        this.logger.log(`[MOCK MODE] SMS verification would be sent to ${phoneNumber} with code: ${verificationCode}`);
      ...
      this.logger.log(`SMS verification sent to ${formattedPhone}: ${result.MessageId}`);
      return { success: true };
    } catch (error) {
