...
describe('SettingController (e2e)', () => {
    ...
    it('should update user profile for PUT /settings/user/:userId/profile', async () => {
        ...
        mockSettingService.updateUserSettings = jest.fn().mockResolvedValue(mockResult);
    ...
    it('should return 200 and user billing address for GET /settings/user/:userId/billing', async () => {
        ...
        mockSettingService.getUserSettingsByCategory = jest.fn().mockResolvedValue(mockBilling);

        const res = await request(app.getHttpServer()).get('/settings/user/123/billing');
        expect(res.status).toBe(200);
    ...
    it('should return 200 and all user settings for GET /settings/user/:userId/all', async () => {
        ...
        mockSettingService.getAllUserSettings = jest.fn().mockResolvedValue(mockAllSettings);

        const res = await request(app.getHttpServer()).get(`/settings/user/${userId}/all`);
        expect(res.status).toBe(200);
    ...
    it('should delete user settings by category for DELETE /settings/user/:userId/category/:category', async () => {
        ...
        mockSettingService.deleteUserSettingsByCategory = jest.fn().mockResolvedValue(undefined);

        const res = await request(app.getHttpServer())
            .delete(`/settings/user/${userId}/category/${category}`);

        expect(res.status).toBe(204);
        expect(mockSettingService.deleteUserSettingsByCategory).toHaveBeenCalledWith(userId, category);
    });

    it('should delete all user settings for DELETE /settings/user/:userId/all', async () => {
        const userId = '123';

        mockSettingService.deleteAllUserSettings = jest.fn().mockResolvedValue(undefined);
    ...
    it('should initialize default settings for POST /settings/seed', async () => {
        mockSettingService.seedDefaultSettings = jest.fn().mockResolvedValue(undefined);
    ...
    it('should return 200 and user profile for frontend compatible for GET /settings/frontend/profile/:userId', async () => {
        ...
        const res = await request(app.getHttpServer()).get('/settings/frontend/profile/123');
        expect(res.status).toBe(200);
        expect(res.body).toEqual(mockProfile);
        expect(mockSettingService.getUserSettingsByCategory).toHaveBeenCalledWith(
            '123',
